...
public class EmployFilterDemo {
	...
	public static void main(String[] args) {
		...
		list.stream().filter((emp) -> emp.getName().startsWith("A")).forEach((arg) ->{System.out.println(arg.getName());});
		list.stream().filter((emp) -> emp.getBand() == 'M').forEach((arg) ->{System.out.println(arg.getName());});
	}
}
